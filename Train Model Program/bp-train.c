...
#define VAL_RESULTS  "Val_Results.dat"
#define VALIDATION   1       /* Set 1 to run valdiation data      */
                             /* Otherwise set it to 0             */
#define VAL_NPAT     12      /* Number of training patterns       */

#define NPAT         116     /* Number of training patterns       */
...
int val_set[];
{
    ...
    fprintf(est_fp,"No.");
    for(j=0; j< ONODES; j++) { 
       fprintf(est_fp,",Actual node=%d,Estimated node=%d,Difference node=%d");
    }
    fprintf(est_fp,"\n");

    for(valnp=0; valnp< VAL_NPAT; ++valnp) {
       k = val_set[valnp];
       fprintf(est_fp,"%d", valnp+1);
       Compute_Hidden_Layer_Activation((val_set[valnp]));
       Compute_Output_Layer_Activation();
       for(j=0; j< ONODES; j++) { 
          fprintf(est_fp,",% lf,% lf,% lf",
            Target_Pattern[k][j], Onode_Act[j], (Target_Pattern[k][j]-Onode_Act[j]));
       }
       fprintf(est_fp,"\n");
    }
    return;
