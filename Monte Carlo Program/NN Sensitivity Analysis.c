...
#define CORENPAT     105        /* Core number of training patterns   */ 
                               /* in the input file                  */
#define REPLICATIONS 100        /* Number of replications             */
 ...
 */
    ...
    for(i=CORENPAT; i< NPAT; i++) {
        for(j=0; j< INODES; j++) {
            k = (i % CORENPAT);
            x = Gaussian_Random_Number(0.0,(NOISE_LEVEL*Input_Pattern[k][j]));
            if (j == (NF_VAR-1)){
               Input_Pattern[i][j] = Input_Pattern[k][j];
            }
            else {
               Input_Pattern[i][j] = Input_Pattern[k][j]+x;
            }
        }
    }
    fclose(input_fp);
...
double x[NPAT];
    ...
    for(np=0;np<nnpat;np++) {
        Input_Pattern[np][(NF_VAR-1)] =  MinLimit[(NF_VAR-1)]+ (MaxLimit[(NF_VAR-1)] - MinLimit[(NF_VAR-1)])*
