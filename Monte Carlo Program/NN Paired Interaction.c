...
#define TARGETS      ""
#define RESULTS      "MgT_O.csv"

#define NF_VAR_ONE   3         /* Index of noise free variable (one)   */
                               /* NF_VAR_ONE = 1,...,INODES            */
#define NF_VAR_TWO   4         /* Index of random change variable (two)*/
                               /* NF_VAR_TWO = 1,...,INODES            */
#define CORENPAT     105        /* Core number of training patterns     */ 
                               /* in the input file                    */
...
double x[NPAT];
    ...
    for(np=0;np<nnpat;np++){
        Input_Pattern[np][(NF_VAR_ONE-1)] =  MinLimit[(NF_VAR_ONE-1)]+
                                         (MaxLimit[(NF_VAR_ONE-1)] - MinLimit[(NF_VAR_ONE-1)])*
                                         Input_Pattern[np][(NF_VAR_ONE-1)];
    }
    fprintf(fmt_fp,"%s,%s,%s,%s,%s\n","Variable","Mean","StdDev","LCL","UCL");
    for(np=0;np< nnpat; np++){
        //fprintf(fmt_fp,"%d",np+1);
        fprintf(fmt_fp,"%lf,%lf,%lf,%lf,%lf",Input_Pattern[np][(NF_VAR_ONE-1)], Mean[np],
                                          StdDev[np], Lcl[np], Ucl[np]);
        // for(n=0;n< REPLICATIONS; n++){
        //     fprintf(fmt_fp,",%lf",estimate[(np+n*nnpat)]); 
        // }
        fprintf(fmt_fp,"\n");
    }
    fclose(fmt_fp);
